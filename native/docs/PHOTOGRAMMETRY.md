...
- **`points3D.txt`** — `POINT3D_ID X Y Z R G B ERROR TRACK[]`. Семенное
  ...
  **Семена нужны не только диапазонам глубин: `stereo_fusion` решает, какие
  снимки перекрываются, по общим точкам разреженной модели.** Измерено на
  синтетическом плоском воркспейсе (`tools/pm_probe.py`, 2026-09-08):
  безупречные карты глубин, но **0 слитых точек без семян и 369 230 с 1521
  семенем**. Поэтому `write_sparse` в режиме dense отказывает по имени, если ни
  одно семя не видно двумя снимками.
...
## 9. sm_120: измерено, обе дороги открыты
...
это sm_120**, которого не существовало, когда те ядра компилировались.

**Проверено 2026-09-08** пробой `tools/pm_probe.py` (синтетическая
текстурированная плоскость 400×400 мм, восемь известных поз, наша же модель
из `colmapio`, затем `image_undistorter → patch_match_stereo → stereo_fusion`
в контейнере и подгонка плоскости к `fused.ply`):

| образ | `patch_match_stereo`, 8 видов 1920 px | слитых точек | ошибка плоскости |
|---|---|---|---|
| `colmap/colmap:latest` (JIT из PTX) | 188 с | карты глубин верны (446.2 мм против 446.0 ожидаемых) | — |
| `orbiter/colmap:cuda129-sm120` | 193 с | 369 230 | медиана 0.000 мм, MAD 0.014, p95 0.049 |

То есть **драйвер JIT-ит compute_90 PTX на 5060 Ti, и публичный образ считает
PatchMatch правильно**; собственный образ даёт тот же результат без JIT и без
зависимости от драйвера. Он собран (`docker images orbiter/colmap`), 2.9 ГБ,
компиляция заняла 11 минут на 7 vCPU Docker Desktop, а не 40–60: GUI, Qt,
OpenGL, ONNX и апстримные тесты выключены.
...
3. **Свой образ уже собран**, `native/docker/colmap-cuda128`: COLMAP 4.2.0 из
   исходников с `-DCMAKE_CUDA_ARCHITECTURES="75;89;120"`, то есть с настоящим
   машинным кодом под sm_120, sm_89 (Ada) и sm_75 (запасная карта); только
   CLI — без GUI, Qt, OpenGL и ONNX. Пересборка — около 15 минут фоновой
   работы (11 минут компиляции на 7 vCPU), которую оператор запускает руками;
   ни один тест и ни один гейт CI её не вызывает. Подробности, включая точные
...
- **Вопрос sm_120 закрыт пробой (§9)**: и публичный образ через JIT, и свой
  `orbiter/colmap:cuda129-sm120` считают PatchMatch на 5060 Ti верно. Откат на
  GTX 1650 SUPER при `max_image_size 1000` остаётся страховкой на случай
  `no kernel image` или нехватки памяти — в логе будет сказано, что случилось.
