...
становится одноглазым `PairSample`. Пока второй глаз видит доску и стоит
(`_last_corners` есть и `_still`), поток ждёт пару — её кадр приходит на
несколько мс позже; глаз вне кадра или дрожащий не останавливает стадию. `gate()` в соло-режиме спрашивает только свой глаз
