...
`_pair_gate(solo)`, так что баннер не говорит «ok», пока захват ждёт пару. `gate()` в соло-режиме спрашивает только свой глаз
(`board / settling / moving / wait / dup / ok`), `gate_report()` — тот же
текст, что и раньше. Стадии гида и их пороги (`guide.py`): линза —
