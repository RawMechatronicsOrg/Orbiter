...
`311363b`. Все константы ниже выписаны из исходников, а не из README; там, где
...
### Вето по консенсусу и центроидное вето

`veto_follow`: проекции кандидатов сдвигаются на медианный сдвиг кадра
(`_veto_offsets` — до ближайшего run'а правой полосы), если `|сдвиг| ≤
veto_follow_max_px = 8` и MAD `≤ veto_follow_mad_px = 2`; иначе на панели
`veto_note` говорит, почему нет. После центроидов — второе вето: проекция
точки листа должна лежать в `confirm_px` от **ближайшего** run'а правой
полосы на той сканлинии (`right_runs`, `nearest_run_residual`), иначе
`n_rejected_offside`. Пиксельное вето с раздутием и ширинами полос
пропускало кандидатов с ~9 px (±7 мм по глубине на 400 мм); центроидное —
±2.3 мм. Полный бюджет ошибок — `docs/ERROR_BUDGET.md`.

### Стерео-уточнение глубины (`refine_by_right`)

После гейтов дальности и прыжков, до перевода в кадр доски. Лист даёт
глубину по одному глазу через базу в `d = 74` мм (смещение листа от левой
камеры): 1 px центроида = `Z²/(f·d)` глубины. Правый глаз видит ту же
полосу через базу пары `B`; его центроид на той сканлинии, куда точка
проецируется, — второе измерение глубины: один шаг Ньютона вдоль левого
луча (`∂v/∂h` считается численно, `project_right` точки и точки + 1 мм по
лучу). Оба измерения несут одну и ту же ошибку левого пикселя — лист через
`Z²/(f·d)`, стерео через `Z²/(f·B)`, — поэтому вес листа `a` минимизирует
дисперсию `a·sheet + (1−a)·stereo` с общим членом, а не смешивает две
«независимые» оценки: при равном шуме центроидов и `B = 2d` выходит чистое
стерео (`a = 0`); остаток пары `rms_px` идёт в шум правого глаза и
возвращает вес листу. `a` зажат в `[0, 1]`. Ворота: `stereo_refine_max_rms_px
= 1.0` (выше — только лист; пара стенда была 2.29 px), окно
`stereo_refine_window_px = 6` вокруг проекции, `stereo_refine_max_shift_mm
= 10`, `MIN_DV_PX_PER_MM = 0.05` (база вдоль полосы — глубины в правом глазу
нет). `ScanFrame.n_refined / refine_shift_mm / refine_share / refine_note`
— на панели SCAN строка `refine`.

### Выдержка (`exposure.ExposureKeeper`)

Красный канал под пикселями полосы теперь едет вместе с ними
(`StripePixels.r`, на GPU собирается там же); `laser.exposure_of` даёт
p90 и долю пикселей ≥ 250, `EyeStats.stripe_peak / stripe_clipped`. Держатель
в своём потоке: целевой пик 200–240, шаг вниз ×0.7 при клипе, вверх ×1.2 при
пике < 200, не чаще `SETTLE_S = 1.5` с, только при включённом лазере; каждые
`VERIFY_S = 5` с читает `/api/controls/{cam}` и `/api/cameras/{cam}`
(`worker.device_reopens`) и переписывает `auto_exposure = 1`,
`power_line_frequency = 1`, `exposure_time_absolute` после reopen или
расхождения. Значения — в `~/.orbiter-native/exposure.json`. Гид на стадиях
LASER и CHECK показывает `STRIPE SATURATED …`.

## 4. Накопление точек

Перед всем — **поза, через которую кадр кладётся в кадр доски**
(`posesmooth.PoseSmoother`, в `ScanWorker._process`). Углы доски на краю
кадра, в блике, под полосой появляются и исчезают от кадра к кадру; каждая
смена набора двигает позу на доли градуса и миллиметр-два при неподвижном
сканере, и каждый такой кадр кладёт ту же поверхность чуть в стороне —
«пух» вокруг неподвижного скана. Рука так не двигается, поэтому кадр
кладётся через **медиану поз** соседей по времени: центрированное окно
`WINDOW = 7` кадров (медиана трансляций по осям; вращения — медиана
rotvec относительно центрального кадра). Одиночный прыжок переголосован,
шум усредняется, ровное движение проходит без запаздывания (медиана
прямого ряда — его середина). Цена — задержка `WINDOW // 2 = 3` кадра;
разрыв `MAX_GAP_S = 0.5` с закрывает окно, хвост кладётся по тому, что
есть. `_place` пересчитывает `points_camera` через новую позу (с той же
rolling-shutter поправкой, если была) и заново применяет объём;
`ScanFrame.pose_smooth_mm` — на сколько сглаженная поза отстоит от своей.
Два гейта до этого: `MIN_POSE_CORNERS = 12` углов на оба глаза и
расхождение собственных поз глаз не больше `POSE_GAP_MAX_DEG = 2` /
`POSE_GAP_MAX_MM = 15` — иначе пара описывает другой риг, кадр не
сканируется, панель говорит почему. И ещё раньше — `_steady`: поза глаза
пересчитывается через углы, которые были в **каждом** из его последних
`STEADY_FRAMES = 12` кадров (длиннее периода полного ChArUco-прохода
`redetect_every = 10`, когда краевые углы приходят и уходят), если таких
не меньше `MIN_STEADY_CORNERS = 12`: одни и те же углы — одна и та же
поза, мигание набора позу не двигает.
...
| `ScanParams.stereo_refine_max_rms_px` | `scan.py` | `1.0` | стерео-уточнение выключено, пока остаток пары выше |
| `ScanParams.stereo_refine_window_px` / `_max_shift_mm` | `scan.py` | `6` / `10` | центроид правого глаза дальше окна или поправка больше — не используется |
| `MIN_DV_PX_PER_MM` | `scan.py` | `0.05` | база вдоль полосы: правый глаз не знает глубины |
...
| `POSE_GAP_MAX_DEG` / `POSE_GAP_MAX_MM` | `scanworker.py` | `2.0` / `15.0` | кадр, где собственные позы глаз расходятся сильнее: пара — с другого рига |
| `MIN_POSE_CORNERS` | `scanworker.py` | `12` | поза по меньшему числу углов на оба глаза не кладёт точки |
| `WINDOW` / `MAX_GAP_S` | `posesmooth.py` | `7` / `0.5` | окно медианы поз и разрыв, закрывающий его |
...
| принятие результата | `PAIR_MOVED_DEG` / `PAIR_MOVED_MM`, `PLANE_MOVED_DEG` / `PLANE_MOVED_MM` | `1.0° / 5 мм`, `1.0° / 3 мм` — новое решение из ≥ `MOVED_MIN_VIEWS = 10` пар / `MOVED_MIN_FRAMES = 6` поз, отстоящее от серверного дальше, замещает его без сравнения счётчиков: геометрия сама говорит, что риг двигали (`_of_another_rig`, `last_note` на панели) |
| темп цикла | `CYCLE_MIN_S` | `4.0` с |
| что решать | `Job.skip` (`CalibrationFlow._unchanged`) | решение с неизменившимся счётчиком данных пропускается (`Outcome.skipped`); пересчитанная линза тянет за собой пару, лист и readout; `Solve now` (`request`) решает всё; `Outcome.timings` — секунды на решение, в строке цикла панели |

> **Правило принятия изменено в `311363b`.** `_better` жёстко отказывает
> претенденту с меньшим объёмом данных: остаток здесь измеряется по тем же
> данным, по которым решение подгонялось, и падает, когда данных меньше.
> Мерилом интринсик стал `sigma_f_px` вместо `rms_px`. Новые интринсики
> списывают зависимые решения (`_retire_dependants`), а зависимые решения
> цикла не сохраняются, если интринсики, через которые они посчитаны,
> отвергнуты и отличаются от действующих (`_stale_dependants`). Гейты кадров
> (`STILL_PX`, `FAST_PX`, новизна, спаривание) не тронуты. Там же появилось
> `ScanFrame.veto_px` — на сколько пикселей глаза расходятся в том, где лежит
> полоса. Это и есть число, показывающее, может ли калибровка сканировать.

### Соло-режим и гид (`CalibrationFlow.solo`, `guide.Guide`)

Автозахват вида требует **пары**: `_capture` берёт `_find_pair()`, и если у
одного глаза доски нет — вида нет. Для линзы это плохо: дисторсия меряется по
углам и краям кадра, а туда второй глаз не смотрит вовсе. Гид на стадиях
линзы выставляет `flow.solo = "left" | "right"`, и тогда неподвижная (тот же
`STILL_PX`) и новая (та же `novelty_threshold`) доска в **одном** этом глазу
становится одноглазым `PairSample`. Пока второй глаз отдал неподвижную
доску не старше `PARTNER_FRESH_S` (два кадра) от кадра своего глаза
(`_partner`), поток ждёт пару — её кадр приходит на несколько мс позже;
глаз вне кадра, дрожащий или с умершим потоком стадию не останавливает.
Новизна в соло-режиме — своего глаза (`novelty(solo, …)`), и для одиночного
вида, и для пары: иначе правый глаз, гуляя по кадру, копил бы дубли левого.
`gate(solo=…)` повторяет тот же выбор: `_solo_gate`, а при живом партнёре —
`_pair_gate(solo)`, так что баннер не говорит «ok», пока захват ждёт пару;
`gate_report()` — тот же текст, что и раньше.

Стадии гида и их пороги (`guide.py`): линза —
`K_VIEWS = 30` видов, `K_CELLS = 24` из 36 клеток сетки `GRID = 6`, разброс
наклона `MIN_TILT_SPREAD`; пара — `PAIR_VIEWS = 20`; лист — `PLANE_FRAMES = 8`
поз; readout — необязателен; проверка — `veto_px <= VETO_OK_PX = 6` (дважды
`confirm_px` скана) на живой паре. Цель на видео — наименее посещённая
клетка, ближайшая к текущему положению доски; слова направления считаются
уже в ориентации показа (`orient.map_point`), а не в координатах сенсора.
